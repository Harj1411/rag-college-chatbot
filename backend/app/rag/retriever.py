...
class RAGRetriever:
    ...
    def retrieve_relevant_chunks(
        ...
        # Fallback: if strict threshold yielded 0, but candidates exist with positive score >= 0.10, return top candidates
        if not valid_chunks and all_retrieved:
            if all_retrieved[0]["score"] >= 0.10:
                valid_chunks = [c for c in all_retrieved if c["score"] >= 0.10][:k]
